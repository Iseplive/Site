				...
				if($filepaths = File::upload('attachment_video')){
					...
					foreach($filepaths as $i => $filepath){
						...
						try {	
							...
							$command=PHPVIDEOTOOLKIT_FFMPEG_BINARY.' -i '.escapeshellarg($filepath).' -vcodec libx264 -profile high -preset ultrafast -r 25 -vb 4000k -maxrate 5000k -bufsize 4000k -vf scale="min(1280\, iw):-1" -threads 0 -acodec libfaac -ab 192k -y '.escapeshellarg($tempfilepath);
							exec($command);
							unlink($filepath);
